...
def count_tokens(text: str, model: str = "gpt-5-mini") -> int:
    try:
        ...
        enc = tiktoken.get_encoding("cl100k_base")  # fallback for new models
    return len(enc.encode(text))


def estimate_cost(tin: int, tout: int, model: str = "gpt-5-mini") -> float:
...
def report(prompt_text: str, reply: str, ms: float, model: str = "gpt-5-mini") -> None:
    tin, tout = count_tokens(prompt_text, model), count_tokens(reply, model)
    print(f"[meter] in={tin} tok  out={tout} tok  cost=$"
          f"{estimate_cost(tin, tout, model):.5f}  latency={ms:.0f}ms")
